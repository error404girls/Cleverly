  ...
  async function generateFlashcards(notes) 
  {
    ...
    try 
    {
      const response = await fetch(
        "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyB_WoqniusYawIVp1l5ueJUD06zuCaIr2o",
        {
          method: "POST",
          headers: {
...
function addFlashcard() {
  const term=prompt("Enter the term (question/keyword):");
  if(!term)
     return;
  const def=prompt("Enter the definition/answer:");
  if(!def)
     return;
  const card=document.createElement("div");
  card.className="card";
...
function editFlashcard() 
{
  ...
  const termElement=card.querySelector(".flashcard-term");
  const defElement=card.querySelector(".flashcard-definition");
  const newTerm=prompt("Edit the term:", termElement.textContent);
  if(newTerm===null) 
    return;
  const newDef=prompt("Edit the definition:",defElement.textContent);
  if(newDef===null) 
    return;
  termElement.textContent=newTerm;
  defElement.textContent=newDef;
  updateDisplayedCard();
}
